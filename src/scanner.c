...
static bool scan_injected_python_expression(TSLexer *lexer) {
  bool any_chars = false;
  bool in_comment = false;
  bool in_apostro_str = false;
  bool in_quote_str = false;
  ...
  while (lexer->lookahead != 0) {
    ...
    any_chars = true;
    lexer->result_symbol = injected_python_expression;
  }
  lexer->advance(lexer, false);
  lexer->mark_end(lexer);
  return any_chars;
...
bool tree_sitter_mako_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
  ...
  if (valid_symbols[injected_python_expression]) return scan_injected_python_expression(lexer);
  return false;
}
